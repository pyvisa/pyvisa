...
simple-to-use \module{visa} module of the PyVISA package.  It implements
...
print itc4.read()
\end{verbatim}
Instead of separate write and read operatons, you can do both with one
...
\begin{memberdesc}{values_format}
  The data format for measurement data sent from the instrument to the
  computer.  See section~\ref{sec:reading-binary-data} for further information.
\end{memberdesc}
...
For GPIB instruments however, it's set to \code{CR+LF}\@.  For RS232 (once it's
supported), it's just \code{LF}\@.
...
\code{delay}.  They are inspired by HTBasic.  \code{send_end} is a boolean.  If
it's \code{True} (the default), the EOI line is asserted after each write
...
\item[single] The values are expected as IEEE floating point values with single
  precision (i.\,e.\ four bytes each).  The data must have a
  prepended~\verb|"#0"| and an appended~\verb|"\n"| (no matter which
  termination character you have set).
