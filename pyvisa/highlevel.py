...
#: :class:`ResourceManager` methods.
#:
#: :interface_type: Interface type of the given resource string.
#: :class:`pyvisa.constants.InterfaceType`
#: :interface_board_number: Board number of the interface of the given resource
#: string.
#: :resource_class: Specifies the resource class (for example, "INSTR") of the
#: given resource string.
#: :resource_name: This is the expanded version of the given resource string.
#:                 The format should be similar to the VISA-defined canonical
#:                 resource name.
...
class VisaLibraryBase(object):
    ...
    A class derived from `VisaLibraryBase` library provides the low-level
    communication to the underlying devices providing Pythonic wrappers to VISA
    functions. But not all derived class must/will implement all methods.
    ...
    In general, you should not instantiate it directly. The object exposed to
    the user is the :class:`pyvisa.highlevel.ResourceManager`. If needed, you
    can access the VISA library from it::
    ...
    #: Its elements are tuples with three elements: The handler itself (a
    #: Python callable), the user handle (as a ct object) and the handler
    #: again, this time as a ct object created with CFUNCTYPE.
    handlers = None
        ...
        if library_path == '':
            ...
            for path in cls.get_library_paths():
                try:
                    ...
                    logger.debug('Could not open VISA library %s: %s',
                                 path, str(e))
                    errs.append(str(e))
    ...
    @staticmethod
    def get_debug_info():
        """Override this method to return an iterable of lines with the backend
        debug details.
    ...
    def get_last_status_in_session(self, session):
        ...
        try:
            ...
            raise errors.Error('The session %r does not seem to be valid as it'
                               ' does not have any last status' % session)
    ...
    def ignore_warning(self, session, *warnings_constants):
        ...
        """
        session_ignored_warnings = self._ignore_warning_in_session[session]
        session_ignored_warnings.update(warnings_constants)
        yield
        session_ignored_warnings.difference_update(warnings_constants)
    ...
    def uninstall_visa_handler(self, session, event_type, handler,
        ...
        for ndx, element in enumerate(self.handlers[session]):
            if (element[0] is handler and element[1] is user_handle and
                    element[4] == event_type):
    ...
    def __uninstall_all_handlers_helper(self, session):
        for element in self.handlers[session]:
            self.uninstall_handler(session, element[4],
                                   element[2], element[1])
        del self.handlers[session]

    def uninstall_all_visa_handlers(self, session):
        """Uninstalls all previously installed handlers for a particular
        session.

        :param session: Unique logical identifier to a session. If None,
                        operates on all sessions.
    ...
    def read_memory(self, session, space, offset, width, extended=False):
        """Reads in an 8-bit, 16-bit, 32-bit, or 64-bit value from the
        specified memory space and offset.

        Corresponds to viIn* functions of the VISA library.

        :param session: Unique logical identifier to a session.
        :param space: Specifies the address space. (Constants.*SPACE*)
        :param offset: Offset (in bytes) of the address or register from which
                       to read.
    ...
    def write_memory(self, session, space, offset, data, width,
                     extended=False):
        """Write in an 8-bit, 16-bit, 32-bit, 64-bit value to the specified
    ...
    def move_in(self, session, space, offset, length, width, extended=False):
        """Moves a block of data to local memory from the specified address
        space and offset.
        ...
        :param offset: Offset (in bytes) of the address or register from which
                       to read.
        :param length: Number of elements to transfer, where the data width of
                       the elements to transfer is identical to the source data
                       width.
    ...
    def move_out(self, session, space, offset, length, data, width,
                 extended=False):
        """Moves a block of data from local memory to the specified address
        space and offset.
        ...
        """
        if width == 8:
            return self.move_out_8(session, space, offset, length, data,
                                   extended)
        elif width == 16:
            return self.move_out_16(session, space, offset, length, data,
                                    extended)
        elif width == 32:
            return self.move_out_32(session, space, offset, length, data,
                                    extended)
        elif width == 64:
            return self.move_out_64(session, space, offset, length, data,
                                    extended)
    ...
    def buffer_read(self, session, count):
        """Reads data from device or interface through the use of a formatted
        I/O read buffer.
    ...
    def close(self, session):
        ...
        :param session: Unique logical identifier to a session, event, or find
                        list.
        :return: return value of the library call.
        :rtype: :class:`pyvisa.constants.StatusCode`
    ...
    def enable_event(self, session, event_type, mechanism, context=None):
        """Enable event occurrences for specified event types and mechanisms in
        a session.
    ...
    def gpib_control_atn(self, session, mode):
        ...
        :param mode: Specifies the state of the ATN line and optionally the
                     local active controller state. (Constants.VI_GPIB_ATN*)
        :return: return value of the library call.
        :rtype: :class:`pyvisa.constants.StatusCode`
    ...
    def gpib_control_ren(self, session, mode):
        ...
        :param mode: Specifies the state of the REN line and optionally the
                     device remote/local state. (Constants.VI_GPIB_REN*)
        :return: return value of the library call.
        :rtype: :class:`pyvisa.constants.StatusCode`
    ...
    def gpib_pass_control(self, session, primary_address, secondary_address):
        ...
        :param secondary_address: Secondary address of the targeted GPIB device
                                  ...
                                  contain the value Constants.VI_NO_SEC_ADDR.
        :return: return value of the library call.
        :rtype: :class:`pyvisa.constants.StatusCode`
    ...
    def map_address(self, session, map_space, map_base, map_size,
        ...
        :param session: Unique logical identifier to a session.
        :param map_space: Specifies the address space to map.
                          (Constants.*SPACE*)
        :param map_base: Offset (in bytes) of the memory to be mapped.
        :param map_size: Amount of memory to map (in bytes).
        :param access:
        :param suggested: If not Constants.VI_NULL (0), the operating system
                          attempts to map the memory to the address specified
                          in suggested. There is no guarantee, however, that
                          the memory will be mapped to that address. This
                          operation may map the memory into an address region
    ...
    def map_trigger(self, session, trigger_source, trigger_destination, mode):
        ...
        :param session: Unique logical identifier to a session.
        :param trigger_source: Source line from which to map.
                               (Constants.VI_TRIG*)
    ...
    def memory_allocation(self, session, size, extended=False):
        ...
        :param extended: Use 64 bits offset independent of the platform.
        :return: offset of the allocated memory, return value of the library
                 call.
    ...
    def memory_free(self, session, offset, extended=False):
        """Frees memory previously allocated using the memory_allocation()
        operation.
    ...
    def move(self, session, source_space, source_offset, source_width,
        ...
        :param source_offset: Offset of the starting address or register from
                              which to read.
        :param source_width: Specifies the data width of the source.
        :param destination_space: Specifies the address space of the
                                  destination.
        :param destination_offset: Offset of the starting address or register
                                   to which to write.
    ...
    def move_asynchronously(self, session, source_space, source_offset,
        ...
        :param length: Number of elements to transfer, where the data width of
                       the elements to transfer is identical to the source data
                       width.
        :return: Job identifier of this asynchronous move operation, return
    ...
    def move_in_8(self, session, space, offset, length, extended=False):
        """Moves an 8-bit block of data from the specified address space and
    ...
    def move_out_8(self, session, space, offset, length, data, extended=False):
        """Moves an 8-bit block of data from local memory to the specified
    ...
    def move_out_16(self, session, space, offset, length, data,
                    extended=False):
        """Moves an 16-bit block of data from local memory to the specified
        address space and offset.
    ...
    def move_out_32(self, session, space, offset, length, data,
                    extended=False):
        """Moves an 32-bit block of data from local memory to the specified
        address space and offset.
    ...
    def move_out_64(self, session, space, offset, length, data,
                    extended=False):
        """Moves an 64-bit block of data from local memory to the specified
        address space and offset.
    ...
    def open(self, session, resource_name,
        ...
        :param open_timeout: Specifies the maximum time period (in
                             milliseconds) that this operation waits before
                             returning an error.
    ...
    def set_buffer(self, session, mask, size):
        """Sets the size for the formatted I/O and/or low-level I/O
        communication buffer(s).

        Corresponds to viSetBuf function of the VISA library.

        :param session: Unique logical identifier to a session.
        :param mask: Specifies the type of buffer. (Constants.VI_READ_BUF,
                     .VI_WRITE_BUF, .VI_IO_IN_BUF, .VI_IO_OUT_BUF)
    ...
    def uninstall_handler(self, session, event_type, handler,
                          user_handle=None):
        """Uninstalls handlers for events.
        ...
        :param handler: Interpreted as a valid reference to a handler to be
                        uninstalled by a client application.
        :param user_handle: A value specified by an application that can be
                            used for identifying handlers uniquely in a session
                            for an event.
        :return: return value of the library call.
        :rtype: :class:`pyvisa.constants.StatusCode`
    ...
    def unmap_trigger(self, session, trigger_source, trigger_destination):
        ...
        :param session: Unique logical identifier to a session.
        :param trigger_source: Source line used in previous map.
                               (Constants.VI_TRIG*)
    ...
    def usb_control_in(self, session, request_type_bitmap_field, request_id,
        ...
        :param request_id: bRequest parameter of the setup stage of a USB
                           control transfer.
        :param request_value: wValue parameter of the setup stage of a USB
                              control transfer.
        ...
        :param length: wLength parameter of the setup stage of a USB control
                       transfer.
                       This value also specifies the size of the data buffer to
                       receive the data from the optional data stage of the
                       control transfer.
        :return: - The data buffer that receives the data from the optional
    ...
    def usb_control_out(self, session, request_type_bitmap_field, request_id,
        ...
        :param index: wIndex parameter of the setup stage of a USB control
                      transfer.
                      This is usually the index of the interface or endpoint.
        :param data: The data buffer that sends the data in the optional data
    ...
    def vxi_command_query(self, session, mode, command):
        """Sends the device a miscellaneous command or query and/or retrieves
        the response to a previous query.
        ...
        :param mode: Specifies whether to issue a command and/or retrieve a
                     response. (Constants.VI_VXI_CMD*, .VI_VXI_RESP*)
    ...
    def write_asynchronously(self, session, data):
        ...
        :param session: Unique logical identifier to a session.
        :param data: data to be written.
        :return: Job ID of this asynchronous write operation, return value of
                 the library call.
        :rtype: jobid, :class:`pyvisa.constants.StatusCode`
...
def list_backends():
    ...
    Backends are installed python packages named pyvisa-<something> where
    <something> is the name of the backend.
...
def get_wrapper_class(backend_name):
    ...
    try:
        ...
        raise ValueError('Wrapper not found: No package named pyvisa-%s' %
                         backend_name)
...
def open_visa_library(specification):
    ...
    wrapper will be created automatically when you create a ResourceManager
    object.
    ...
    if not specification:
        try:
            ...
            logger.debug('No visa libaray specified and environment variable '
                         'PYVISA_LIBRARY is unset. Using NI library')
...
class ResourceManager(object):
    ...
    :param visa_library: VisaLibrary Instance, path of the VISA library or
                         VisaLibrary spec string. (if not given, the default
    ...
    @classmethod
    def register_resource_class(cls, interface_type, resource_class,
                                python_class):
    ...
    def __new__(cls, visa_library=''):
        ...
        if visa_library.resource_manager is not None:
            ...
            logger.debug('Reusing ResourceManager with session %s',
                         obj.session)
            return obj
    ...
    def resource_info(self, resource_name, extended=True):
        ...
        if extended:
            ret, err = self.visalib.parse_resource_extended(self.session,
                                                            resource_name)
        else:
    ...
    def open_resource(self, resource_name,
        ...
        :type access_mode: :class:`pyvisa.constants.AccessModes`
        :param open_timeout: time out to open.
        :param resource_pyclass: resource python class to use to instantiate
                                 the Resource. Defaults to None: select based
                                 on the resource name.
        :param kwargs: keyword arguments to be used to change instrument
        ...
        if resource_pyclass is None:
            ...
            try:
                ...
                resource_pyclass =\
                    self._resource_classes[(constants.InterfaceType.unknown,
                                            '')]
                logger.warning('There is no class defined for %r. '
                               'Using Resource', (info.interface_type,
                                                  info.resource_class))
