...
def _query_capability_key(resource_spec: ResourceSpec) -> str:
    return resource_spec.query_capability_key or (
        f"resource.query.{resource_spec.contract_suffix.replace('::', '.')}"
    )
...
def test_message_based_timeout_attribute_contract(
    ...
    timeout_capability_key = (
        f"resource.timeout.{resource_spec.contract_suffix.replace('::', '.')}"
    )
    if not pyvisa_backend_capabilities.get(timeout_capability_key, True):
        pytest.skip(
